---
id: mcp-licensing
url: metadata/mcp/getting-started/licensing
title: Licensing
weight: 7
description: "The GroupDocs.Metadata MCP server runs in evaluation mode out of the box; unlock it with your existing GroupDocs.Metadata license file or with metered (pay-per-use) keys, and confirm the active mode with the get_license_status tool."
...
| **Evaluation** | nothing | Only the first 5 document properties are readable; XMP and EXIF are partly unavailable |
...
{{< alert style="warning" >}}
**Both keys are required.** With only one set, the server ignores the metered configuration and stays in evaluation mode — it reports this explicitly rather than failing silently. Check with [`get_license_status`]({{< ref "metadata/mcp/tools-reference/get-license-status.md" >}}).
{{< /alert >}}
...
Ask your agent *"what is the license status of the metadata server?"*. The [`get_license_status`]({{< ref "metadata/mcp/tools-reference/get-license-status.md" >}}) tool (server **26.9.0+**) answers without processing a document:
...
* [Purchase GroupDocs.Metadata](https://purchase.groupdocs.com/pricing/metadata/net) · [Product page](https://products.groupdocs.com/metadata/net/)
