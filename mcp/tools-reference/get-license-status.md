...
url: metadata/mcp/tools-reference/get-license-status
...
| `server` / `engine` | MCP server version and the underlying GroupDocs.Metadata engine version |
| `note` | Why licensing did not end up where you asked — present only when something needs attention |

On this server `"mode": "evaluation"` has a sharp consequence: **only the first five document properties are readable**, so every read is silently truncated. See [Licensing]({{< ref "metadata/mcp/getting-started/licensing.md" >}}).
