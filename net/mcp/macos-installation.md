...
url: metadata/net/mcp/macos-installation
title: Install on macOS
weight: 4
description: "Install the GroupDocs.Metadata MCP server on macOS (Intel and Apple Silicon) via Homebrew-based bootstrap and the guided installer — registered into Claude Desktop and other clients automatically."
...
Install the GroupDocs.Metadata MCP server on **macOS** — Intel and Apple Silicon — via the Homebrew-based bootstrap and the guided installer, registered into Claude Desktop and other clients automatically:
...
pwsh ./install-groupdocs-mcp.ps1 -Channel docker \
  -Products metadata -Clients claude-desktop,cursor -Verify
```
...
Client config locations on macOS: Claude Desktop `~/Library/Application Support/Claude/claude_desktop_config.json`, VS Code user-level `~/Library/Application Support/Code/User/mcp.json`, Cursor `~/.cursor/mcp.json` — full list in [Register in AI clients]({{< ref "metadata/net/mcp/install-in-ai-clients.md" >}}).
...
The handshake lists the server's six tools; with a document in your storage folder it also runs a real `get_document_info` call through the engine.
...
Installing on a different OS? [Windows]({{< ref "metadata/net/mcp/windows-installation.md" >}}) · [Linux]({{< ref "metadata/net/mcp/linux-installation.md" >}})
