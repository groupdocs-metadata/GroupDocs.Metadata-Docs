...
url: metadata/net/mcp/windows-installation
title: Install on Windows
weight: 2
description: "To install the GroupDocs.Metadata MCP server on Windows, run the prerequisite bootstrapper and the guided installer in PowerShell — it registers the server in Claude Desktop, VS Code, and other clients automatically."
...
To install the GroupDocs.Metadata MCP server on **Windows**, run the prerequisite bootstrapper and the guided installer in PowerShell — it registers the server in Claude Desktop, VS Code, and other clients automatically:
...
```powershell
./install-groupdocs-mcp.ps1 -Channel nuget -Products metadata `
...
Client config locations on Windows: Claude Desktop `%APPDATA%\Claude\claude_desktop_config.json`, VS Code user-level `%APPDATA%\Code\User\mcp.json` — full list with JSON blocks in [Register in AI clients]({{< ref "metadata/net/mcp/install-in-ai-clients.md" >}}).
...
A passing run performs the MCP handshake and prints the server's six tools; if a document exists in your storage folder, it also runs a real `get_document_info` call against it. Exit code `0` means everything works.
...
Installing on a different OS? [Linux]({{< ref "metadata/net/mcp/linux-installation.md" >}}) · [macOS]({{< ref "metadata/net/mcp/macos-installation.md" >}})
