...
url: metadata/net/mcp/linux-installation
title: Install on Linux
weight: 3
description: "Install the GroupDocs.Metadata MCP server on Linux with the setup bootstrapper (installs PowerShell, Docker or the .NET 10 SDK with libgdiplus and libfontconfig1) and the guided installer."
keywords: install MCP server Linux, MCP server Ubuntu, libgdiplus MCP error, self-hosted document metadata Linux
productName: GroupDocs.Metadata MCP Server for .NET
toc: True
---

Install the GroupDocs.Metadata MCP server on **Linux** with the setup bootstrapper — it installs PowerShell (`pwsh`) plus Docker, or the .NET 10 SDK with `libgdiplus` and `libfontconfig1` — and then the guided installer:
...
pwsh ./install-groupdocs-mcp.ps1 -Channel docker \
  -Products metadata -Clients claude-desktop -Verify
```
...
Client config locations on Linux: Claude Desktop `~/.config/Claude/claude_desktop_config.json`, VS Code user-level `~/.config/Code/User/mcp.json` — full list in [Register in AI clients]({{< ref "metadata/net/mcp/install-in-ai-clients.md" >}}).
...
* **Fonts look wrong in cleaned and updated files** — install `ttf-mscorefonts-installer` (the setup script pre-accepts its EULA via debconf).

Installing on a different OS? [Windows]({{< ref "metadata/net/mcp/windows-installation.md" >}}) · [macOS]({{< ref "metadata/net/mcp/macos-installation.md" >}})
